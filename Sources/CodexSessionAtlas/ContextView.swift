...
struct ContextCompositionView: View {
    ...
    private var filtered: [ContextMaterial] {
        let values = (intervalOnly ? change?.materials ?? [] : materials).filter {
            (category == nil || $0.category == category)
                && (search.isEmpty || ($0.localizedTitle() + $0.preview).localizedCaseInsensitiveContains(search))
        }
        switch ordering {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    HStack(alignment: .top) {
                        VStack(alignment: .leading, spacing: 6) {
                            Text(L("查看窗口使用率，追查已记录的内容及其变化。"))
                                .font(.system(size: 12)).foregroundStyle(MonitorStyle.secondary)
                        }
                        Spacer()
                        Button { showReadingGuide.toggle() } label: {
                            Label(L("这些数字怎么看"), systemImage: "questionmark.circle").font(.system(size: 12))
                        }.buttonStyle(MonitorButtonStyle(selected: showReadingGuide))
                    }
                    if showReadingGuide {
                        VStack(alignment: .leading, spacing: 10) {
                            Text(L("上下文使用率：运行时报告的已用 Token ÷ 窗口容量，是最近采样时的容量使用情况。"))
                            Text(L("已记录内容的文本占比：某类可读字符数 ÷ 所选范围全部可读字符数，不是 Token 占比。"))
                            Text(L("Token 消耗：多次请求的用量统计。已有输入可能重复计入，因此累计值可大于窗口容量。"))
                    ...
                    if !history.complete && !history.phases.isEmpty {
                        Label(L("历史内容尚未读完（\(Int(Double(history.readBytes) / Double(max(history.totalBytes, 1)) * 100))%）。阶段、采样和组成暂时只覆盖已读取部分。"), systemImage: "clock.arrow.circlepath")
                    ...
                    if let phase {
                        phaseRail.id(model.language)
                        capacityPanel(phase).id(model.language)
                        changePanel.id(model.language)
                        compositionPanel(phase).id(model.language)
                        HStack(spacing: 10) {
                            Button { intervalOnly = false; ordering = "文本最长"; category = nil; search = ""; reader.scrollTo("materials", anchor: .top) } label: {
                                Label(L("查看最长内容"), systemImage: "chart.bar.xaxis")
                            }
                            if change != nil {
                                Button { intervalOnly = true; ordering = "时间顺序"; category = nil; search = ""; reader.scrollTo("materials", anchor: .top) } label: {
                                    Label(L("看两次采样间新增"), systemImage: "plus.rectangle.on.rectangle")
                                }
                            }
                            Spacer(minLength: 0)
                        }.font(.callout)
                        materialPanel.id(model.language).id("materials")
                        usagePanel.id(model.language)
                    } else {
                        MonitorPanel {
                            VStack(alignment: .leading, spacing: 10) {
                                if model.contextLoading { ProgressView().controlSize(.small) }
                                Text(model.contextLoading ? L("正在读取上下文内容") : L("尚未读到上下文内容")).font(.headline)
                                Text(history.note.map { Localization.diagnostic($0) } ?? L("读取本地消息和压缩记录后，这里会显示各阶段包含的内容。")).foregroundStyle(MonitorStyle.secondary)
                                Button(L("重新读取")) { model.loadComposition() }
                            }
                        }
                    }
                    if !history.complete && !history.phases.isEmpty {
                        HStack {
                            ...
                            Text(L("已读取 \(Int(Double(history.readBytes) / Double(max(history.totalBytes, 1)) * 100))% 的本地记录"))
                            Spacer()
                            Button(L("继续读取")) { model.loadComposition() }.disabled(model.contextLoading)
                        }.font(.caption).foregroundStyle(MonitorStyle.secondary)
                    }
                    if let note = history.note { Label(Localization.diagnostic(note), systemImage: "info.circle").font(.caption).foregroundStyle(MonitorStyle.amber) }
                }.monitorDocument()
            }
        }
    ...
    private var phaseRail: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text(L("选择压缩阶段")).font(.headline)
                Spacer()
                Text(L("\(history.phases.count) 个阶段 · 每次压缩开始一个新阶段")).font(.caption).foregroundStyle(MonitorStyle.secondary)
            }
                        ...
                        Button {
                            ...
                            HStack(spacing: 10) {
                                ...
                                VStack(alignment: .leading, spacing: 5) {
                                    Text(L(value.title)).font(.system(size: 13, weight: .semibold))
                                    Text(L("\(clockLabel(value.startedAt)) · \(value.materials.count) 项内容")).font(.system(size: 11))
                                }
                                if value.id == history.phases.last?.id { Text(history.complete ? L("当前") : L("已读末段")).font(.caption.weight(.semibold)) }
            ...
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top) {
                    VStack(alignment: .leading, spacing: 7) {
                        Text(L("上下文使用率")).font(.headline)
                        HStack(alignment: .firstTextBaseline, spacing: 9) {
                            Text(checkpoint?.ratio.map { "\(Int($0 * 100))%" } ?? L("未知"))
                                .font(.system(size: 34, weight: .semibold, design: .default)).monospacedDigit()
                            Text(L("已用")).font(.callout).foregroundStyle(MonitorStyle.secondary)
                            if let point = checkpoint {
                                Text("\(tokenLabel(point.used)) / \(point.capacity.map(tokenLabel) ?? L("未知"))")
                                    .font(.callout).monospacedDigit()
                            }
                        }
                        if let point = checkpoint, let capacity = point.capacity, capacity > 0 {
                            Text(L("按本次采样，剩余 \(tokenLabel(max(0, capacity - point.used))) Token"))
                                .font(.caption).foregroundStyle(MonitorStyle.secondary)
                        }
                    }
                    Spacer()
                    VStack(alignment: .trailing, spacing: 7) {
                        MonitorBadge(text: checkpointID == nil ? (history.complete ? L("最近采样") : L("已读部分的最近采样")) : L("正在回看"), color: MonitorStyle.accent)
                        if let point = checkpoint {
                            Text(timestampLabel(point.timestamp)).font(.caption).monospacedDigit()
                            Text(point.model == "未知" ? L("未知") : point.model).font(.caption).foregroundStyle(MonitorStyle.secondary).lineLimit(1)
                        }
                    }
                }
                if !phase.checkpoints.isEmpty { capacityChart(phase) }
                HStack(spacing: 8) {
                    Text(L("查看到")).font(.caption).foregroundStyle(MonitorStyle.secondary)
                    Picker(L("查看时刻"), selection: $checkpointID) {
                        Text(L("最新记录（持续更新）")).tag(nil as UInt64?)
                    ...
                    Button { stepCheckpoint(-1) } label: { Image(systemName: "chevron.left") }
                        .disabled(!canStep(-1)).accessibilityLabel(L("上一采样"))
                    Button { stepCheckpoint(1) } label: { Image(systemName: "chevron.right") }
                        .disabled(!canStep(1)).accessibilityLabel(L("下一采样"))
                    Spacer(minLength: 0)
                    if checkpointID != nil { Button(L("回到最新")) { checkpointID = nil; intervalOnly = false } }
                }
                Text(L("点击曲线可回看。使用率来自运行时采样；下方内容来自本地日志，可能含当次输出，不等于模型请求的完整输入。"))
                    .font(.caption).foregroundStyle(MonitorStyle.secondary)
            }
        }
    }
    ...
    private func capacityChart(_ phase: ContextPhase) -> some View {
        Chart {
            ForEach(phase.checkpoints) { point in
                if let ratio = point.ratio {
                    LineMark(x: .value(L("时间"), point.timestamp), y: .value(L("占用 %"), ratio * 100),
                             series: .value(L("模型与容量"), "\(point.model):\(point.capacity ?? 0)"))
                        .foregroundStyle(MonitorStyle.accent).lineStyle(StrokeStyle(lineWidth: 2.5))
                }
            }
            if let point = checkpoint, let ratio = point.ratio {
                RuleMark(x: .value(L("所选时刻"), point.timestamp)).foregroundStyle(MonitorStyle.secondary.opacity(0.4))
                    .lineStyle(StrokeStyle(lineWidth: 1, dash: [4, 4]))
                PointMark(x: .value(L("时间"), point.timestamp), y: .value(L("占用 %"), ratio * 100))
        ...
        .frame(height: 145).accessibilityLabel(L("上下文占用曲线；也可通过查看时刻菜单或前后按钮选择采样"))
    }
    ...
    private var changePanel: some View {
        MonitorPanel {
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    Label(L("相邻采样之间发生了什么"), systemImage: "arrow.left.arrow.right").font(.headline)
                    Spacer()
                ...
                if let change {
                    HStack(alignment: .top, spacing: 24) {
                        metric(L("占用变化"), value: change.tokenDelta.map { ($0 > 0 ? "+" : "") + tokenLabel($0) + " Token" } ?? L("不可直接比较"))
                        metric(L("新增记录"), value: L("\(change.materials.count) 项"))
                        metric(L("新增可读文本"), value: characterLabel(change.materials.reduce(0) { $0 + $1.characters }))
                    }
                    Text(change.tokenDelta == nil
                         ? L("模型或窗口容量不同，暂不计算 Token 差值；内容仍按两次采样之间的日志位置列出。")
                         : L("这些内容在两次采样之间出现，可用于追查变化；字符量不能换算为精确 Token 贡献。"))
                        .font(.caption).foregroundStyle(MonitorStyle.secondary)
                } else {
                    Text(L("这是本阶段的首个采样，或尚无采样；没有同阶段的前一次记录可供比较。"))
    ...
    private func compositionPanel(_ phase: ContextPhase) -> some View {
        MonitorPanel {
            VStack(alignment: .leading, spacing: 16) {
                HStack {
                    VStack(alignment: .leading, spacing: 6) {
                        Text(L("已记录内容的文本占比")).font(.headline)
                        Text(L("\(materials.count) 项 · \(characterLabel(totalCharacters)) · \(materials.filter { !$0.readable }.count) 项正文不可读"))
                            .font(.caption).foregroundStyle(MonitorStyle.secondary)
                    }
                    Spacer()
                    if category != nil { Button(L("清除分类筛选")) { category = nil } }
                }
                            ...
                            ForEach(categories, id: \.self) { type in
                                ...
                                if count > 0 {
                                    Rectangle().fill(MonitorStyle.category(type))
                                        ...
                                        .help("\(L(type.label)) · \(percentLabel(count, total: totalCharacters)) · \(characterLabel(count))")
                                }
                            }
                ...
                HStack(spacing: 16) {
                    Label(L("压缩后保留 \(materials.filter(\.fromReplacement).count) 项"), systemImage: "archivebox")
                    Label(L("本阶段新增 \(materials.filter { !$0.fromReplacement }.count) 项"), systemImage: "plus.circle")
                }.font(.caption).foregroundStyle(MonitorStyle.secondary)
                Text(L("点击类别查看内容。占比按可读字符计算，不是 Token 占比。"))
                    .font(.caption).foregroundStyle(MonitorStyle.secondary)
                if phase.number > 0 {
                    Label(phase.hasReplacement ? L("保留内容来自压缩后记录的消息集合；加密摘要无法读取正文。")
                          : L("日志没有提供压缩后的完整消息集合，只显示已知摘要与新增内容。"), systemImage: "info.circle")
                        .font(.caption).foregroundStyle(MonitorStyle.amber)
                }
                Text(L("覆盖范围：本地日志可见内容。未记录的系统提示、工具定义、服务端裁剪与加密正文不在可读统计中。"))
                    .font(.caption).foregroundStyle(MonitorStyle.secondary)
            }
        }
    }
    private func categoryButton(_ type: ContextCategory) -> some View {
        ...
        return Button { category = selected ? nil : type; intervalOnly = false; visibleCount = 50 } label: {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    HStack {
                        Text(L(type.label)).font(.system(size: 13, weight: .semibold))
                        Spacer(minLength: 4)
                        Text(values.allSatisfy { !$0.readable } ? L("不可读") : percentLabel(count, total: totalCharacters))
                            .font(.system(size: 12, weight: .semibold)).monospacedDigit()
                    }
                    Text(L("\(values.count) 项 · \(values.allSatisfy { !$0.readable } ? L("正文不可读") : characterLabel(count))"))
            ...
            .help(L(type.explanation))
            .accessibilityLabel(L("\(L(type.label))，\(values.count) 项，\(characterLabel(count))，\(selected ? L("已选中") : L("点击筛选"))"))
    }
    ...
    private var materialPanel: some View {
        MonitorPanel {
            VStack(alignment: .leading, spacing: 14) {
                HStack {
                    Text(L("内容明细")).font(.headline)
                    Text(L("\(filtered.count) 项匹配")).font(.caption).foregroundStyle(MonitorStyle.secondary)
                    Spacer()
                    Picker(L("排序"), selection: $ordering) {
                        Text(L("时间顺序")).tag("时间顺序")
                        Text(L("最新在前")).tag("最新在前")
                        Text(L("文本最长")).tag("文本最长")
                    }.frame(width: model.language.resolved == .english ? 210 : 175).controlSize(.large)
                }
                HStack(spacing: 8) {
                    Button(L("截至所选时刻")) { intervalOnly = false; visibleCount = 50 }
                        .buttonStyle(MonitorButtonStyle(selected: !intervalOnly))
                    Button(L("两次采样间新增")) { intervalOnly = true; visibleCount = 50 }
                        .buttonStyle(MonitorButtonStyle(selected: intervalOnly)).disabled(change == nil)
                    Spacer(minLength: 0)
                    MonitorSearchField(placeholder: L("搜索内容"), text: $search).frame(maxWidth: 230)
                }
                if let category {
                    HStack {
                        CategoryBadge(category: category)
                        Button(L("清除筛选")) { self.category = nil; search = "" }
                        Spacer()
                    }
                    Text(L(category.explanation)).font(.system(size: 12)).foregroundStyle(MonitorStyle.secondary)
                }
                if filtered.isEmpty { Text(L("这个范围没有匹配内容。可以清除分类、搜索，或改看截至所选时刻的全部内容。")).font(.callout).foregroundStyle(MonitorStyle.secondary).padding(.vertical, 12) }
                ...
                if filtered.count > visibleCount {
                    Button { visibleCount += 50 } label: { Text(L("再显示 50 条")).frame(maxWidth: .infinity) }
                }
            }
        }
    }
    private func materialRow(_ material: ContextMaterial) -> some View {
        Button { model.inspect(material) } label: {
            MonitorRecordLabel(title: material.localizedTitle(),
                               preview: material.localizedPreview(), category: material.category,
                               label: material.category.label, timestamp: material.timestamp,
                               metadata: (material.fromReplacement ? L("压缩后保留") : L("本阶段新增")) + " · "
                                   + (material.readable ? characterLabel(material.characters) : L("正文不可读")))
        ...
        MonitorPanel {
            VStack(alignment: .leading, spacing: 14) {
                Button { showUsage.toggle() } label: {
                    HStack {
                        Label(L("Token 消耗"), systemImage: "sum").font(.headline)
                        Spacer()
                        Image(systemName: showUsage ? "chevron.up" : "chevron.down")
                    }.frame(maxWidth: .infinity)
                }.buttonStyle(MonitorButtonStyle(row: true))
                if showUsage {
                    Text(L("以下是任务最新用量，不随上方历史时刻切换。每次请求都可能再次计入已有输入，因此累计值可能大于窗口容量。"))
                        .font(.caption).foregroundStyle(MonitorStyle.secondary)
                    HStack(alignment: .top, spacing: 20) {
                        usage(L("最近一次请求"), model.observation.usage.last?.tokens)
                        usage(L("当前轮次"), model.observation.turnUsage)
                        usage(L("任务累计"), model.observation.cumulative)
                    }
                    Text(L("缓存输入属于输入，推理输出属于输出，不能再次相加；消耗不是费用估算。"))
                        .font(.caption).foregroundStyle(MonitorStyle.secondary)
                }
            }
        }
    ...
    private func usage(_ label: String, _ tokens: TokenBreakdown?) -> some View {
        VStack(alignment: .leading, spacing: 9) {
            ...
            Text(tokens.map { tokenLabel($0.total) + " Token" } ?? L("未知")).font(.title3).monospacedDigit()
            if let tokens {
                Label(L("输入 \(tokenLabel(tokens.input))"), systemImage: "arrow.down.circle.fill").foregroundStyle(MonitorStyle.category(.user))
                Text(L("其中缓存 \(tokenLabel(tokens.cached))")).foregroundStyle(MonitorStyle.secondary)
                Label(L("输出 \(tokenLabel(tokens.output))"), systemImage: "arrow.up.circle.fill").foregroundStyle(MonitorStyle.category(.assistant))
                Text(L("其中推理 \(tokenLabel(tokens.reasoning))")).foregroundStyle(MonitorStyle.secondary)
            }
