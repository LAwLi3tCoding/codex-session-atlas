...
struct SessionDetail: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .top, spacing: 15) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    HStack(spacing: 8) {
                        Label(session.workspaceName, systemImage: "folder").help(session.workspace)
                        Text("·")
                        Text(session.model)
                        Text("推理强度 \(session.effort)")
                    }.font(.system(size: 11)).foregroundStyle(MonitorStyle.secondary)
                    if let note = session.activityNote {
                        Text(note).font(.system(size: 11)).foregroundStyle(MonitorStyle.secondary)
                    }
                }
                Spacer()
                MonitorBadge(text: session.state.label, color: stateColor(session.state))
                Menu {
                    Button("在 Codex 打开任务") { model.openTask(session.id) }
                    Button("复制任务标识") { copyText(session.id) }
                    if let parent = session.parentID { Button("查看父任务") { model.select(parent) } }
    ...
    private func tab(_ title: String, icon: String) -> some View {
        ...
        }.buttonStyle(MonitorButtonStyle(selected: selected)).accessibilityLabel(title)
    }
    ...
    private var activitySummary: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text("最近活动").font(.system(size: 12, weight: .medium)).foregroundStyle(MonitorStyle.secondary)
                Spacer()
                Text(ageLabel(latestEvent?.timestamp ?? session.lastEventAt)).font(.system(size: 11)).foregroundStyle(MonitorStyle.secondary)
            }
            Text(latestEvent.map(friendlyTool) ?? "尚无执行记录")
                .font(.system(size: 21, weight: .medium)).lineLimit(2)
            let preview = latestEvent.map { RecordExcerpt($0.preview).text } ?? ""
            Text(preview.isEmpty ? "有新的本地执行记录时，会自动显示在这里。" : preview)
                .font(.system(size: 12)).foregroundStyle(MonitorStyle.secondary).lineLimit(2)
                .frame(maxWidth: .infinity, alignment: .leading)
            Spacer(minLength: 0)
            Button { model.detailTab = "轨迹" } label: {
                HStack { Text("查看轨迹"); Image(systemName: "arrow.right") }.font(.system(size: 12, weight: .medium))
            }.padding(.leading, -12)
    ...
    private var contextSummary: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text("上下文使用率").font(.system(size: 12, weight: .medium)).foregroundStyle(MonitorStyle.secondary)
                Spacer()
                Text(session.context.map { "采样于 " + clockLabel($0.timestamp) } ?? "尚无采样")
                    .font(.system(size: 11)).foregroundStyle(MonitorStyle.secondary)
            }
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Text(session.context?.ratio.map { "\(Int($0 * 100))%" } ?? "未知")
                    .font(.system(size: 28, weight: .medium)).monospacedDigit()
                if let point = session.context {
                    Text("\(tokenLabel(point.used)) / \(point.capacity.map(tokenLabel) ?? "未知") Token")
            ...
            if let point = session.context, let ratio = point.ratio {
                ...
                Text(point.capacity.map { "剩余约 \(tokenLabel(max(0, $0 - point.used))) Token" } ?? "剩余容量未知")
                    .font(.system(size: 11)).foregroundStyle(MonitorStyle.secondary)
            } else {
                Text("运行时尚未提供窗口使用率。").font(.system(size: 12)).foregroundStyle(MonitorStyle.secondary)
            }
            Spacer(minLength: 0)
            Button { model.detailTab = "上下文" } label: {
                HStack { Text("查看上下文"); Image(systemName: "arrow.right") }.font(.system(size: 12, weight: .medium))
            }.padding(.leading, -12)
        }.frame(height: 180, alignment: .topLeading)
    }
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                MonitorPanel {
                    VStack(alignment: .leading, spacing: 15) {
                        HStack {
                            Text("最近的执行记录").font(.headline)
                            Spacer()
                            Button("全部轨迹") { model.detailTab = "轨迹" }.font(.caption)
                        }
                        let recent = model.events.filter { $0.kind != .reasoning }.prefix(6)
                        if recent.isEmpty { Text("任务有新的持久化记录时，会自动显示在这里。").foregroundStyle(MonitorStyle.secondary).font(.callout) }
                        ForEach(Array(recent)) { event in
                            eventRow(event, compact: true)
                        }
                    }
                }
                if let note = session.dataNote { Label(note, systemImage: "info.circle").font(.caption).foregroundStyle(MonitorStyle.secondary) }
                Text("状态来自最近的本地记录。长时间没有新记录时，需要回到原任务确认；「本轮结束」不等于任务目标已完成。")
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack {
                    ...
                    Button { showResolved.toggle() } label: {
                        Label("显示已解除", systemImage: showResolved ? "checkmark.square.fill" : "square")
                    ...
                    HStack(alignment: .top, spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            HStack {
                                ...
                                Spacer()
                                MonitorBadge(text: item.resolved ? "已解除" : (item.definite ? "已确认" : "待核实"), color: MonitorStyle.amber)
                            }
                            Text(item.explanation).font(.system(size: 12)).foregroundStyle(MonitorStyle.secondary)
                            Text(item.advice).font(.system(size: 12)).fixedSize(horizontal: false, vertical: true)
                            HStack(spacing: 14) {
                                Button("查看依据") {
                                ...
                                Button("打开原任务") { model.openTask(session.id) }
                                Spacer()
                                if !item.seen { Button("我已看过") { model.markSeen(item.id) } }
                            }.font(.caption)
                        }
                    }.padding(.vertical, 14)
                }
    ...
    private var filteredEvents: [TraceEvent] {
        let values = model.events.filter { event in
            (kind == "全部" || event.kind.label == kind) && (!failuresOnly || event.failed)
                && (eventSearch.isEmpty || (event.title + event.preview).localizedCaseInsensitiveContains(eventSearch))
        }
        switch eventOrder {
    ...
    private var trajectory: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                Text("按时间查看执行过程，也可以筛选失败、较大输出或较长耗时。")
                    .font(.system(size: 12)).foregroundStyle(MonitorStyle.secondary)
                MonitorPanel {
                    VStack(alignment: .leading, spacing: 14) {
                        HStack {
                            Text("执行记录").font(.system(size: 15, weight: .semibold))
                            Text("已加载 \(model.events.count) 条 · 匹配 \(filteredEvents.count) 条").font(.caption).foregroundStyle(MonitorStyle.secondary)
                            Spacer()
                            Button { failuresOnly.toggle() } label: {
                                Label("失败 \(model.events.filter(\.failed).count)", systemImage: "exclamationmark.circle")
                            }.buttonStyle(MonitorButtonStyle(tint: .red, selected: failuresOnly))
                        }
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack(spacing: 6) {
                                Button("全部类型") { kind = "全部" }.buttonStyle(MonitorButtonStyle(selected: kind == "全部"))
                                ForEach(TraceKind.allCases.filter { type in model.events.contains { $0.kind == type } }, id: \.self) { type in
                                    Button { kind = kind == type.label ? "全部" : type.label } label: {
                                        Label("\(type.label) \(model.events.filter { $0.kind == type }.count)", systemImage: icon(type))
                        ...
                        HStack(spacing: 12) {
                            MonitorSearchField(placeholder: "搜索记录", text: $eventSearch)
                            Picker("排序", selection: $eventOrder) {
                                ForEach(["最新在前", "最早在前", "输出最大", "耗时最长"], id: \.self) { Text($0).tag($0) }
                            }.frame(width: 180).controlSize(.large)
                        }
                        DisclosureGroup("记录范围与说明") {
                            VStack(alignment: .leading, spacing: 10) {
                                if let oldest = model.events.map(\.timestamp).min(), let latest = model.events.map(\.timestamp).max() {
                                    Text("已加载范围：\(timestampLabel(oldest)) — \(timestampLabel(latest))")
                                }
                                Text("分类数量和排序仅针对已加载记录。工具记录可能合并调用与结果；未提供耗时或体积时保持未知。")
                                HStack(spacing: 15) {
                                    Label("调用待返回", systemImage: "arrow.up.right.square.fill").foregroundStyle(MonitorStyle.category(.toolCall))
                                    Label("工具已有记录", systemImage: "arrow.down.left.square.fill").foregroundStyle(MonitorStyle.category(.toolResult))
                                    Label("明确失败", systemImage: "exclamationmark.circle.fill").foregroundStyle(.red)
                                }
                            }.padding(.top, 10).frame(maxWidth: .infinity, alignment: .leading)
                        }.font(.caption).foregroundStyle(MonitorStyle.secondary)
                    }
                }
                if let note = model.pageNote { Label(note, systemImage: "info.circle").font(.caption).foregroundStyle(MonitorStyle.amber) }
                MonitorPanel {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        if events.isEmpty { Text("没有匹配记录。可以调整类型、失败筛选或搜索，再读取更早内容。").foregroundStyle(MonitorStyle.secondary).padding(.vertical, 15) }
                        ForEach(Array(events.enumerated()), id: \.element.id) { index, event in
                            VStack(alignment: .leading, spacing: 10) {
                                if ["最新在前", "最早在前"].contains(eventOrder), index == 0 || events[index - 1].turnID != event.turnID {
                                    HStack {
                                        Image(systemName: "bubble.left.and.bubble.right")
                                        Text(event.turnID.isEmpty ? "轮次标识未提供" : "轮次 \(event.turnID.prefix(8))")
                        ...
                        if matching.count > visibleEventCount {
                            Button("显示更多已加载记录（\(matching.count - visibleEventCount) 条）") { visibleEventCount += 100 }
                        } else if model.hasMore {
                            Button { visibleEventCount += 100; model.more() } label: {
                                Text(model.loadingHistory ? "正在读取…" : "读取更早的记录").frame(maxWidth: .infinity)
    ...
    private func eventRow(_ event: TraceEvent, compact: Bool = false) -> some View {
        ...
        let details = [
            event.durationMs.map { String(format: "%.2f 秒", $0 / 1000) },
            event.outputBytes > 0 ? "输出 " + ByteCountFormatter.string(fromByteCount: Int64(event.outputBytes), countStyle: .binary) : nil,
            event.status == "requested" ? "结果待记录" : nil
        ].compactMap { $0 }.joined(separator: " · ")
        return Button { model.inspect(event) } label: {
            MonitorRecordLabel(title: friendlyTool(event), preview: event.preview,
    ...
    private var collaborators: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                Text("谁在一起完成这个任务").font(.system(size: 21, weight: .semibold, design: .default))
                Text("每个 Agent 有独立上下文。点击任务查看它自己的上下文内容和执行记录。").font(.callout).foregroundStyle(MonitorStyle.secondary)
                if let parent = session.parentID { Button("查看父任务") { model.select(parent) } }
                MonitorPanel {
                    VStack(alignment: .leading, spacing: 12) {
                            ...
                            Button { if agent.id != session.id { model.select(agent.id) } } label: {
                                HStack(spacing: 12) {
                                    ...
                                    VStack(alignment: .leading, spacing: 5) {
                                        ...
                                        Text("\(agent.model) · \(ageLabel(agent.lastEventAt))").font(.caption).foregroundStyle(MonitorStyle.secondary)
                                        if let point = agent.context { Text("上下文 \(point.ratio.map { "\(Int($0 * 100))%" } ?? "未知") · 采样于 \(clockLabel(point.timestamp))").font(.caption).foregroundStyle(MonitorStyle.secondary) }
                                    }
                                    Spacer()
                        ...
                        if children.isEmpty { Text("没有记录到直接子任务。").font(.caption).foregroundStyle(MonitorStyle.secondary).padding(.top, 8) }
                    }
                }
            }.monitorDocument()
        }.background(MonitorStyle.canvas)
