...
struct MonitorWorkspace: View {
    ...
    var body: some View {
            ...
            GeometryReader { geometry in
                HStack(spacing: 0) {
                    if geometry.size.width >= 950 || !showDetailOnCompact {
                        taskList.frame(width: geometry.size.width >= 950 ? (model.language.resolved == .english ? 320 : 280) : nil)
                    }
                    if geometry.size.width >= 950 || showDetailOnCompact {
                        VStack(spacing: 0) {
                            if geometry.size.width < 950 {
                                HStack {
                                    Button { showDetailOnCompact = false } label: { Label(L("任务列表"), systemImage: "chevron.left") }
                                    Spacer()
                            ...
                            if let session = model.selected {
                                ...
                                VStack(spacing: 13) {
                                    ...
                                    Text(L("选一个任务，看看它的执行过程")).font(.title3)
                                    Text(L("所有本地任务仍会持续监控。")).foregroundStyle(MonitorStyle.secondary)
        ...
        .sheet(isPresented: $model.showAbout) { MonitorAbout(model: model) }
    ...
    private var topbar: some View {
        HStack(spacing: 9) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text(L("会话 · 轨迹 · 上下文")).font(.system(size: 10)).foregroundStyle(MonitorStyle.secondary)
            }
            Spacer(minLength: 0)
            Button { model.showSettings = true } label: { Image(systemName: "slider.horizontal.3") }
                .accessibilityLabel(L("监控设置"))
    ...
    private var taskList: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    Text(L("任务")).font(.system(size: 12, weight: .medium))
                    ...
                    Menu {
                        Picker(L("排序"), selection: $sortMode) {
                            ForEach(SessionSortMode.allCases, id: \.self) { mode in Text(L(mode.label)).tag(mode) }.id(model.language)
                        }
                        Divider()
                        Toggle(L("显示系统任务"), isOn: $showSystem)
                        Button(scope == "已归档" ? L("返回全部任务") : L("查看已归档任务")) { scope = scope == "已归档" ? "全部会话" : "已归档" }
                ...
                HStack(spacing: 7) {
                    ...
                    TextField(L("搜索任务、项目或模型"), text: $search).textFieldStyle(.plain).font(.system(size: 12))
                ...
                HStack(spacing: 5) {
                    ...
                    scopeButton("最近活跃", value: "最近活跃")
                        .help(L("轮次尚未结束，且近 5 分钟有执行记录；不包含等待用户的任务。"))
                    scopeButton(L("需关注"), value: "待关注")
                    if scope == "已归档" { MonitorBadge(text: "已归档") }
                }
                Menu {
                    Button(L("全部项目")) { project = nil }
                    ForEach(Array(Set(model.snapshot.sessions.map(\.workspace))).sorted(), id: \.self) { path in
                        Button(path == "未知项目" ? L("未知项目") : path) { project = path }
                    }
                } label: {
                    HStack {
                        Image(systemName: "folder").foregroundStyle(MonitorStyle.secondary)
                        Text(project.map { $0 == "未知项目" ? L("未知项目") : URL(fileURLWithPath: $0).lastPathComponent } ?? L("全部项目")).lineLimit(1)
                        Spacer()
                        Image(systemName: "chevron.down").font(.system(size: 9))
                    }.font(.system(size: 12)).foregroundStyle(MonitorStyle.secondary).frame(minHeight: 36).contentShape(Rectangle())
                }.menuStyle(.borderlessButton)
                Text(L("排序：\(L(sortMode.label))"))
                    .font(.system(size: 10)).foregroundStyle(MonitorStyle.secondary)
                    .help(L("按整组最新活动排序，子任务活动也会让父任务上移。尚无执行记录时使用目录时间。可在右上角筛选菜单切换。"))
            }.padding(.horizontal, 14).padding(.vertical, 10)
            let rows = displayedRows
            if rows.isEmpty {
                VStack(spacing: 10) {
                    Text(model.snapshot.sessions.isEmpty ? L("正在读取任务目录") : L("没有匹配的任务")).font(.headline)
                    Text(model.snapshot.diagnostics.first.map { Localization.diagnostic($0) } ?? L("调整搜索或筛选条件。")).font(.caption).foregroundStyle(MonitorStyle.secondary)
                        ...
                        ForEach(rows) { row in
                            HStack(alignment: .top, spacing: 4) {
                                if row.childCount > 0 {
                                    ...
                                    }.buttonStyle(.plain).accessibilityLabel(L("展开或收起子任务"))
                                }
                                Button {
                                    ...
                                    TaskRow(session: row.session, title: model.title(row.session), children: row.childCount,
                                            activityAt: row.activityAt, language: model.language,
                            ...
                            .contextMenu {
                                Button(L("在 Codex 打开")) { model.openTask(row.id) }
                                Button(L("复制任务标识")) { copyText(row.id) }
                            }
                        }
                    }.padding(.vertical, 9)
                }
    ...
    private func scopeButton(_ title: String, value: String) -> some View {
        Button { scope = value } label: {
            HStack(spacing: 4) {
                Text(L(title)).lineLimit(1).minimumScaleFactor(0.85)
                if value == "待关注" {
    ...
    private var footer: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(model.snapshot.diagnostics.isEmpty ? L("持续观测 · 本机只读") : L("部分数据待恢复"))
                    .help(model.snapshot.diagnostics.map { Localization.diagnostic($0) }.joined(separator: "\n"))
                if let time = model.snapshot.refreshedAt {
                    Text(L("更新于 \(clockLabel(time)) · v\(Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "—")"))
                        .monospacedDigit()
                }
            }
            Spacer(minLength: 0)
            Button { Task { await model.refresh() } } label: { Image(systemName: "arrow.clockwise") }
                .accessibilityLabel(L("立即刷新"))
        }.font(.system(size: 10)).foregroundStyle(MonitorStyle.secondary)
        ...
        return VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .top, spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 7) {
                    ...
                    Text(model.detailMaterial?.localizedTitle() ?? friendlyTool(event))
                        .font(.system(size: 18, weight: .semibold)).lineLimit(2)
                    if let material = model.detailMaterial {
                        Text((material.fromReplacement ? L("压缩后保留") : L("本阶段新增")) + " · "
                        ...
                        Text((event.durationMs.map { String(format: L("%.2f 秒"), $0 / 1000) } ?? L("耗时未提供"))
                             + " · " + (event.outputBytes > 0 ? L("输出 ") + Int64(event.outputBytes).formatted(.byteCount(style: .binary).locale(AppLanguage.current.locale)) : L("输出体积未提供")))
                            .font(.system(size: 11)).foregroundStyle(MonitorStyle.secondary)
                    }
                }
                Spacer(minLength: 0)
                Button { model.detailEvent = nil } label: { Image(systemName: "xmark") }
                    .keyboardShortcut(.cancelAction).accessibilityLabel(L("关闭内容"))
            }.padding(28)
            ScrollView {
                MonitorRecordBody(text: model.detailText, raw: model.rawDetail, code: code)
                    .padding(.horizontal, 28).padding(.vertical, 12)
            }
            DisclosureGroup(L("来源与记录信息")) {
                VStack(alignment: .leading, spacing: 8) {
                    if let material = model.detailMaterial {
                        Text(L(material.category.explanation))
                    }
                    Text(event.source.path).textSelection(.enabled)
                    HStack {
                        if let offset = event.source.offset { Text(L("记录位置 \(offset)")).monospacedDigit() }
                        Spacer()
                        Button(L("复制来源位置")) { copyText(event.source.path + (event.source.offset.map { L("\n记录字节位置: \($0)") } ?? "")) }
                    }
                }.font(.system(size: 11)).foregroundStyle(MonitorStyle.secondary).padding(.top, 8)
            }.font(.system(size: 11)).foregroundStyle(MonitorStyle.secondary)
                .padding(.horizontal, 28).padding(.top, 18)
            HStack(spacing: 6) {
                Button(L("正文")) { model.rawDetail = false }
                    .buttonStyle(MonitorButtonStyle(selected: !model.rawDetail))
                Button(L("原始记录")) { model.rawDetail = true }
                    .buttonStyle(MonitorButtonStyle(selected: model.rawDetail))
                    .disabled(model.detailMaterial?.readable == false)
                Spacer()
                if model.detailHasMore { Button(L("下一页")) { model.nextDetailPage() } }
                Button { copyText(model.detailText) } label: { Label(L("复制内容"), systemImage: "doc.on.doc") }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            HStack(spacing: 5) {
                ...
                Text(contextOnly ? L("含活跃子任务") : L(session.state.label))
                Spacer()
            ...
            HStack {
                Text(session.workspace == "未知项目" ? L("未知项目") : session.workspaceName).lineLimit(1)
                if children > 0 { Text(L("· \(children) 个子任务")) }
                Spacer()
                Text(ageLabel(activityAt))
                    .help(children > 0 ? L("该任务及筛选范围内子任务的最近活动：\(timestampLabel(activityAt))") : L("最近活动：\(timestampLabel(activityAt))"))
            ...
            .help(contextOnly ? L("此父任务仅用于展示活跃子任务的归属关系。") : Localization.diagnostic(session.activityNote ?? session.dataNote ?? ""))
    }
}
